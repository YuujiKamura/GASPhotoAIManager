...
import {
  GitBranch, FileText, Eye, Layers, Cpu, CheckCircle, RefreshCw, Zap, ArrowRight
} from 'lucide-react';
    ...
    <div className="space-y-4">
      <div className="bg-purple-50 border border-purple-200 rounded p-3 text-sm">
        <div className="flex items-center gap-2 font-medium text-purple-800">
          <GitBranch size={16} />AI処理パイプライン（編集可能）
        </div>
                ...
                {step.optional && (
                  <label className="flex items-center gap-1 ml-auto cursor-pointer">
                    <input
                      ...
                      onChange={() => state.handleFlowToggle(step.id)}
                      className="rounded"
                    />
      ...
      <div className="border rounded p-4 bg-gray-50">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          <span className={`px-2 py-1 rounded ${state.flowSettings.detect ? 'bg-yellow-100' : 'bg-gray-200 line-through'}`}>
            🔍 黒板判定
          </span>
          <ArrowRight size={14} className="text-gray-400" />
