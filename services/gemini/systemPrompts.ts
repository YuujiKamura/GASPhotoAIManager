...
*   **判定基準**（黒板キーワードベース）:
    ...
    3. **新規入場者教育実施状況**: 黒板に「新規入場者教育」等の記載がある
    4. **安全巡視状況**: 黒板に「安全巡視」等の記載がある
    5. **保安施設設置状況**: カラーコーン、バリケード、看板等の設置写真
    6. **点灯確認状況**: 保安灯の点灯確認写真

*   **remarksCategory正規化ルール**（データ一貫性のため必ず従うこと）:
    - 「朝礼状況」「朝礼・KYミーティング実施状況」→ **「朝礼実施状況」** に統一
    - 「危険予知活動状況」「KYミーティング実施状況」→ **「KY活動状況」** に統一
    - 「新規入場者教育状況」→ **「新規入場者教育実施状況」** に統一
...
export const REMARKS_CATEGORIES = [
  ...
  // 安全管理（正規化済み - 重複排除）
  "朝礼実施状況",           // 正規形（朝礼状況、朝礼・KYミーティング実施状況 → これに統一）
  "KY活動状況",             // 正規形（危険予知活動状況、KYミーティング実施状況 → これに統一）
  "新規入場者教育実施状況", // 正規形
  "保安施設設置状況",
  "点灯確認状況",
  "安全巡視状況",
