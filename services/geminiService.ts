...
const PRIMARY_MODEL = 'gemini-3.0-flash'; // Fallback, actual value comes from getPrimaryModel()
...
*   **フォーマット規則**:
    - 項目名を先に記載し、その後に設計値・実測値を記載
    - 複数測点がある場合: "基準高 設計H1=50,H2=50,H3=50 実測H1=50,H2=50,H3=51"
    - 単一値の場合: "砕石厚 設計30 実測30" または "t=50mm"
    - 複数項目がある場合は改行で区切る
*   温度測定: "到着温度 161.1℃" など
*   出来形測定: "基準高 設計H1=50,H2=50 実測H1=50,H2=51", "厚さ 設計50 実測52"
*   密度測定: "締固め度 98.5%" など
*   測定値が見えない場合は空文字列 "" を返す
  ...
  const schema: Schema = {
    ...
    items: {
      ...
      properties: {
        ...
        measurements: {
          type: Type.STRING,
          description: "測定値。形式: 「項目名 設計XX 実測XX」。例: 「基準高 設計H1=50,H2=50 実測H1=50,H2=51」「砕石厚 設計30 実測30」「到着温度 161.1℃」。複数項目は改行区切り。値がない場合は空文字"
        },
