...
async function processImage(filePath, options = {}) {
  ...
  const outputMetadata = await (0, import_sharp.default)(outputBuffer).metadata();
  return {
...
async function optimizeImageForPdf(bytes, options) {
  ...
  const image = (0, import_sharp.default)(Buffer.from(bytes));
  const metadata = await image.metadata();
...
function isYoloAvailable(modelPath) {
  ...
  return (0, import_fs.existsSync)(model) && (0, import_fs.existsSync)(script);
}
...
function getTempImageDir() {
  if (!tempImageDir) {
    ...
    if (!(0, import_fs2.existsSync)(tempImageDir)) {
      (0, import_fs2.mkdirSync)(tempImageDir, { recursive: true });
    }
  }
  return tempImageDir;
}
function runClaudeCode(prompt, imagePaths, onLog) {
  ...
  if (imagePaths && imagePaths.length > 0) {
    ...
    for (const p of imagePaths) {
      ...
      (0, import_fs2.copyFileSync)(p, tempPath);
...
${prompt}`;
  ...
  if (testCmd.length > MAX_CMD_LENGTH) {
    ...
    try {
      const result = (0, import_child_process2.spawnSync)("claude", ["--output-format", "text"], {
        input: fullPrompt,
        encoding: "utf-8",
        timeout: 12e4,
        maxBuffer: 10 * 1024 * 1024,
    ...
    cmd = testCmd;
    try {
      const result = (0, import_child_process2.execSync)(cmd, {
        encoding: "utf-8",
        timeout: 12e4,
        maxBuffer: 10 * 1024 * 1024,
        shell: true,
...
async function analyzeCommand(folder, options) {
  ...
  try {
    ...
    if (!stat5.isDirectory()) {
      console.error(import_chalk.default.red(`\u30A8\u30E9\u30FC: ${folderPath} \u306F\u30C7\u30A3\u30EC\u30AF\u30C8\u30EA\u3067\u306F\u3042\u308A\u307E\u305B\u3093`));
      process.exit(1);
    }
  } catch {
    console.error(import_chalk.default.red(`\u30A8\u30E9\u30FC: \u30D5\u30A9\u30EB\u30C0\u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093: ${folderPath}`));
    process.exit(1);
  }
  const scanSpinner = (0, import_ora.default)("\u5199\u771F\u3092\u30B9\u30AD\u30E3\u30F3\u4E2D...").start();
  let imagePaths;
  try {
  ...
  if (imagePaths.length === 0) {
    console.log(import_chalk.default.yellow("\u89E3\u6790\u3059\u308B\u5199\u771F\u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3067\u3057\u305F"));
    process.exit(0);
  }
  ...
  if (options.mode === "construction") {
    try {
      ...
      console.log(import_chalk.default.gray("\u5DE5\u7A2E\u30DE\u30B9\u30BF\u3092\u8AAD\u307F\u8FBC\u307F\u307E\u3057\u305F"));
    } catch {
      console.log(import_chalk.default.yellow("\u5DE5\u7A2E\u30DE\u30B9\u30BF\u306E\u8AAD\u307F\u8FBC\u307F\u306B\u5931\u6557\uFF08\u30C7\u30D5\u30A9\u30EB\u30C8\u52D5\u4F5C\u3067\u7D9A\u884C\uFF09"));
    }
  }
...
\u30E2\u30FC\u30C9: ${options.mode} (Claude Code CLI\u4F7F\u7528)`));
  if (yoloEnabled) {
    console.log(import_chalk.default.gray(`YOLO\u524D\u51E6\u7406: \u6709\u52B9 (\u95BE\u5024=${yoloConfThreshold})`));
  }
  if (useStep1Cache) {
    console.log(import_chalk.default.gray(`Step1\u30AD\u30E3\u30C3\u30B7\u30E5: ${step1CachePath}`));
  }
  if (options.instruction) {
    console.log(import_chalk.default.gray(`\u6307\u793A: ${options.instruction}`));
  }
  console.log("");
  let analyzeSpinner = (0, import_ora.default)("AI\u89E3\u6790\u4E2D...").start();
  let results;
  try {
    results = await analyzePhotos(photoInputs, {
      ...
      onLog: (msg, type) => {
        if (type === "error") {
          ...
          analyzeSpinner = (0, import_ora.default)("AI\u89E3\u6790\u4E2D...").start();
  ...
  const outputSpinner = (0, import_ora.default)("\u51FA\u529B\u30C7\u30FC\u30BF\u3092\u6E96\u5099\u4E2D...").start();
  const outputData = [];
  ...
  const saveSpinner = (0, import_ora.default)("\u7D50\u679C\u3092\u4FDD\u5B58\u4E2D...").start();
  try {
  ...
  console.log(import_chalk.default.gray("\u30B5\u30DE\u30EA\u30FC:"));
  ...
  console.log(import_chalk.default.gray(`
\u51FA\u529B\u30D5\u30A1\u30A4\u30EB: ${outputPath}`));
  console.log(import_chalk.default.gray(`\u6B21\u306E\u30B9\u30C6\u30C3\u30D7: gaspm export ${options.output} -f both
`));
}
async function analyzeViaServer(folder, options) {
  ...
  try {
    ...
    console.error(import_chalk.default.red("\u30A8\u30E9\u30FC: \u30B5\u30FC\u30D0\u30FC\u304C\u8D77\u52D5\u3057\u3066\u3044\u307E\u305B\u3093"));
    console.error(import_chalk.default.yellow("\u5148\u306B `gaspm server start` \u3092\u5B9F\u884C\u3057\u3066\u304F\u3060\u3055\u3044\n"));
    process.exit(1);
  }
  const spinner = (0, import_ora.default)("\u30B5\u30FC\u30D0\u30FC\u306B\u89E3\u6790\u30EA\u30AF\u30A8\u30B9\u30C8\u3092\u9001\u4FE1\u4E2D...").start();
  try {
    ...
    console.log(import_chalk.default.green("\n\u2705 \u89E3\u6790\u304C\u30B5\u30FC\u30D0\u30FC\u3067\u958B\u59CB\u3055\u308C\u307E\u3057\u305F"));
    console.log(import_chalk.default.gray(`   \u30D5\u30A9\u30EB\u30C0: ${folderPath}`));
    console.log(import_chalk.default.gray(`   \u30C0\u30C3\u30B7\u30E5\u30DC\u30FC\u30C9\u3067\u9032\u6357\u3092\u78BA\u8A8D: http://localhost:3001/web-analyzer.html
`));
  } catch (error) {
    spinner.fail("\u89E3\u6790\u30EA\u30AF\u30A8\u30B9\u30C8\u5931\u6557");
    const errMsg = error instanceof Error ? error.message : "Unknown error";
    console.error(import_chalk.default.red(`\u30A8\u30E9\u30FC: ${errMsg}
`));
    process.exit(1);
  }
}
...
async function generatePdfBuffer(photos, options = {}) {
  ...
  let japaneseFont;
  try {
    const fk = import_fontkit.default.default || import_fontkit.default;
      ...
      if (photo.base64) {
        try {
          ...
          page.drawRectangle({
            ...
            borderColor: (0, import_pdf_lib.rgb)(0.7, 0.7, 0.7),
            borderWidth: 0.5
          });
        ...
        lines.forEach((textLine, lineIdx) => {
          ...
          if (lineY > rowY + 5) {
            page.drawText(textLine, {
              ...
              size: fontSize,
              font: japaneseFont,
              color: (0, import_pdf_lib.rgb)(0.1, 0.1, 0.1)
            });
          }
        });
...
async function exportCommand(input, options) {
  ...
  try {
    ...
    if (error.code === "ENOENT") {
      console.error(import_chalk2.default.red(`\u30A8\u30E9\u30FC: \u30D5\u30A1\u30A4\u30EB\u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093: ${inputPath}`));
    } else {
      console.error(import_chalk2.default.red(`\u30A8\u30E9\u30FC: \u30D5\u30A1\u30A4\u30EB\u306E\u8AAD\u307F\u8FBC\u307F\u306B\u5931\u6557: ${error}`));
    }
    process.exit(1);
  }
  ...
  if (aliasConfig.enabled) {
    ...
    if (modifiedCount > 0) {
      console.log(import_chalk2.default.cyan(`\u30A8\u30A4\u30EA\u30A2\u30B9\u9069\u7528: ${modifiedCount}\u4EF6\u3092\u5909\u63DB`));
    }
  }
  console.log(import_chalk2.default.gray(`\u5165\u529B: ${inputPath}`));
  console.log(import_chalk2.default.gray(`\u5199\u771F\u6570: ${inputData.length}\u679A`));
  console.log(import_chalk2.default.gray(`\u5F62\u5F0F: ${options.format}`));
  console.log(import_chalk2.default.gray(`\u30DA\u30FC\u30B8\u3042\u305F\u308A: ${options.photosPerPage}\u679A`));
  if (options.format === "pdf" || options.format === "both") {
    console.log(import_chalk2.default.gray(`PDF\u753B\u8CEA: ${pdfQuality} - ${qualityLabels[pdfQuality]}`));
  }
  if (aliasConfig.enabled) {
    const presetName = aliasConfig.activePreset || "\u30AB\u30B9\u30BF\u30E0";
    console.log(import_chalk2.default.gray(`\u30A8\u30A4\u30EA\u30A2\u30B9: ${presetName}`));
  }
  console.log("");
  ...
  for (const format of formats) {
    if (format === "excel") {
      ...
      console.error(import_chalk2.default.yellow(`\u8B66\u544A: \u4E0D\u660E\u306A\u5F62\u5F0F: ${format}`));
    }
  }
  console.log(import_chalk2.default.green("\n\u2705 \u30A8\u30AF\u30B9\u30DD\u30FC\u30C8\u5B8C\u4E86\n"));
}
async function exportExcel(data, outputDir, baseName, dateStr, photosPerPage, title) {
  const spinner = (0, import_ora2.default)("Excel\u30D5\u30A1\u30A4\u30EB\u3092\u751F\u6210\u4E2D...").start();
  try {
...
async function exportPdf(data, outputDir, baseName, dateStr, photosPerPage, title, fontPath, pdfQuality = "high") {
  const spinner = (0, import_ora2.default)("PDF\u30D5\u30A1\u30A4\u30EB\u3092\u751F\u6210\u4E2D...").start();
  try {
    const photoData = data.map((item) => ({
      fileName: item.fileName,
      base64: item.base64,
...
async function configCommand(action, value) {
  ...
  switch (action) {
    ...
    case "path":
      ...
      break;
    default:
      console.log(import_chalk3.default.yellow(`\u4E0D\u660E\u306A\u30A2\u30AF\u30B7\u30E7\u30F3: ${action}`));
      console.log("");
...
async function handleSetKey(apiKey) {
  if (!apiKey) {
    console.error(import_chalk3.default.red("\u30A8\u30E9\u30FC: API\u30AD\u30FC\u3092\u6307\u5B9A\u3057\u3066\u304F\u3060\u3055\u3044"));
    console.log("\u4F7F\u7528\u65B9\u6CD5: gaspm config set-key <your-api-key>");
    process.exit(1);
  }
  if (!apiKey.startsWith("AIza") || apiKey.length < 30) {
    console.error(import_chalk3.default.yellow("\u8B66\u544A: API\u30AD\u30FC\u306E\u5F62\u5F0F\u304C\u6B63\u3057\u304F\u306A\u3044\u53EF\u80FD\u6027\u304C\u3042\u308A\u307E\u3059"));
    console.log('Gemini API\u30AD\u30FC\u306F\u901A\u5E38 "AIza" \u3067\u59CB\u307E\u308B39\u6587\u5B57\u306E\u6587\u5B57\u5217\u3067\u3059');
  }
  try {
    await setApiKey(apiKey);
    console.log(import_chalk3.default.green("\u2705 API\u30AD\u30FC\u3092\u4FDD\u5B58\u3057\u307E\u3057\u305F"));
    console.log(import_chalk3.default.gray(`\u30AD\u30FC: ${maskApiKey(apiKey)}`));
    console.log(import_chalk3.default.gray(`\u5834\u6240: ${getConfigPath()}`));
  } catch (error) {
    console.error(import_chalk3.default.red("\u30A8\u30E9\u30FC: API\u30AD\u30FC\u306E\u4FDD\u5B58\u306B\u5931\u6557\u3057\u307E\u3057\u305F"));
    console.error(error);
    process.exit(1);
  }
}
async function handleShow() {
  ...
  console.log(import_chalk3.default.gray("API\u30AD\u30FC:"));
  if (envKey) {
    console.log(`  \u74B0\u5883\u5909\u6570 (GEMINI_API_KEY): ${import_chalk3.default.green(maskApiKey(envKey))}`);
  } else {
    console.log(`  \u74B0\u5883\u5909\u6570 (GEMINI_API_KEY): ${import_chalk3.default.gray("\u672A\u8A2D\u5B9A")}`);
  }
  if (config.apiKey) {
    console.log(`  \u8A2D\u5B9A\u30D5\u30A1\u30A4\u30EB: ${import_chalk3.default.green(maskApiKey(config.apiKey))}`);
  } else {
    console.log(`  \u8A2D\u5B9A\u30D5\u30A1\u30A4\u30EB: ${import_chalk3.default.gray("\u672A\u8A2D\u5B9A")}`);
  }
  console.log("");
  if (effectiveKey) {
    console.log(`  ${import_chalk3.default.green("\u2713")} \u6709\u52B9\u306AAPI\u30AD\u30FC: ${maskApiKey(effectiveKey)}`);
  } else {
    console.log(`  ${import_chalk3.default.red("\u2717")} API\u30AD\u30FC\u304C\u8A2D\u5B9A\u3055\u308C\u3066\u3044\u307E\u305B\u3093`);
    console.log("");
    console.log(import_chalk3.default.yellow("API\u30AD\u30FC\u306E\u8A2D\u5B9A\u65B9\u6CD5:"));
    console.log("  1. gaspm config set-key <your-api-key>");
    console.log("  2. \u74B0\u5883\u5909\u6570 GEMINI_API_KEY \u3092\u8A2D\u5B9A");
  }
  console.log("");
  console.log(import_chalk3.default.gray(`\u8A2D\u5B9A\u30D5\u30A1\u30A4\u30EB: ${getConfigPath()}`));
  console.log("");
}
function handlePath() {
...
function openBrowser(url) {
  ...
  (0, import_child_process3.exec)(cmd, (err) => {
...
async function runAnalyzeWeb(folderPath, options = {}) {
  ...
  const projectRoot = process.cwd();
  app.use(import_express.default.static(projectRoot));
...
var import_cors2 = __toESM(require("cors"), 1);
...
`);
  ...
  const projectRoot = process.cwd();
  app.use(import_express2.default.static(projectRoot));
  app.get("/api/health", (_req, res) => {
    res.json({ status: "ok", timestamp: (/* @__PURE__ */ new Date()).toISOString() });
  });
  app.get("/api/logs", (req, res) => {
